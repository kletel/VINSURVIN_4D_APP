...
$caseId:=KST_web_Lire_param(->$tVnom; ->$tVal; "caseId")
$choice:=KST_web_Lire_param(->$tVnom; ->$tVal; "choice")
$parPlat:=KST_web_Lire_param(->$tVnom; ->$tVal; "parPlat")
...
Case of 
	: ($caseId="restaurant")
		...
		$plats:=KST_web_Lire_param(->$tVnom; ->$tVal; "plats")
		$requete:="Vous êtes un caviste expert (30 ans d’expérience). Je vais manger "+$plats+". À partir de cette carte de réstaurant, conseil moi 3 vins le moins cher et le meillure qualité prix. Retourne moi le résultat en JSON avec : nom, couleur, prix,appellation,region ."
		
	: ($caseId="rayon")
		
		$plats:=KST_web_Lire_param(->$tVnom; ->$tVal; "plats")
		$requete:="Vous êtes un caviste expert (30 ans d’expérience). Je vais manger "+$plats+". À partir de ce rayon du vin dans une grande surface, conseil moi 3 vins le moins cher (prix moyen dans une grande surface) et le meillure qualité prix. Retourne moi le résultat en JSON avec : nom, couleur, prix,appellation,region ."
		
	: ($caseId="plat")
		
		$plats:=KST_web_Lire_param(->$tVnom; ->$tVal; "plats")
		
		If ($choice="acheter")  //Acheter un nouveau vin
			
			$budget:=KST_web_Lire_param(->$tVnom; ->$tVal; "budget")
			
			
			If ($parPlat="true")
				$requete:="Vous êtes un caviste expert (30 ans d’expérience). Je vais manger "+$plats+". À partir de mes choix des plats, conseil moi un vin par plat et le meillure qualité prix selon ce budget total de "+$budget+" €. Pas d'explication. Retourne moi le résultat en un tableau de JSON [] avec :plat(plat que j'ai donné), nom, couleur, prix,appellation,region ."
			Else 
				$requete:="Vous êtes un caviste expert (30 ans d’expérience). Je vais manger "+$plats+". À partir de mes choix des plats, conseil moi qu'un seul vin et le meillure qualité prix selon ce budget "+$budget+" €. Pas d'explication. Retourne moi le résultat en un tableau de JSON []  avec : plat(affiche que plat que j'ai donné),nom, couleur, prix,appellation,region ."
			End if 
			
		Else   //Prendre dans la cave
			
			$lstVin:=ds:C1482.Cave.all().toCollection("Nom,Appellation,Région,Reste_en_Cave")
			
			
			//$lstSpec:={Nom: $lstVin.Nom; Région: $lstVin.Région; Appelation: $lstVin.Appellation; Reste_en_Cave: $lstVin.Reste_en_Cave}
			
			$jsonVin:=JSON Stringify:C1217($lstVin)
			
			If ($parPlat="true")
				$requete:="Vous êtes un caviste expert (30 ans d’expérience). Je vais manger "+$plats+". À partir de mes choix des plats et cette liste des vins et nombre de bouteille qui reste dans la cave (si 0 ne le prend pas) "+$jsonVin+" , conseil moi un vin par plat qui est le plus adapté à mes plats.Pas d'explication. Retourne moi le résultat en un tableau de JSON []  avec :plat(plat que j'ai donné), nom, couleur,appellation,region."
			Else 
				$requete:="Vous êtes un caviste expert (30 ans d’expérience). Je vais manger "+$plats+". À partir de mes choix des plats et cette liste des vins et nombre de bouteille qui reste dans la cave (si 0 ne le prend pas) "+$jsonVin+" ,conseil moi qu'un seul vin qui est le plus adapté à mon plat.Pas d'explication. Retourne moi le résultat en un tableau de JSON []  avec :plat(plat que j'ai donné), nom, couleur,appellation,region."
			End if 
	...
	Else 
		...
		$budget:=KST_web_Lire_param(->$tVnom; ->$tVal; "budget")
		$bouteille:=KST_web_Lire_param(->$tVnom; ->$tVal; "bouteille")
		$lstVin:=ds:C1482.Cave.all().toCollection("Nom,Appellation,Région,Reste_en_Cave")
		
		$jsonVin:=JSON Stringify:C1217($lstVin)
		
		$requete:="Vous êtes un caviste expert (30 ans d’expérience). Je souhaite acheter "+$bouteille+" bouteilles du vin avec un budget total de "+$budget+"€. Voici ma cave de vin et nombre de bouteille qui reste dans la cave  , "+$jsonVin+".À partie de cette image de rayon du vin,conseil moi des vins qu'ils ne sont que dans l'image pour completer ma cave.Utilise le prix moyen dans une grande surface s'ils ne sont pas affichés sur l'image. Pas d'explication. Retourne moi le résultat e"+"n un tableau de JSON []  avec :nom, couleur, prix, quantité(suit strictement le nombre de bouteilles à acheter), appellation,region."
...
//La réponse nous envoie un tableau de JSON
ARRAY OBJECT:C1221($result; 0)
JSON PARSE ARRAY:C1219($json; $result)


$jsonSend:=JSON Stringify array:C1228($result)
